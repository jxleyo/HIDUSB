...
Exit:
     ...
     */
    (VOID)InterlockedExchange((PVOID)&pMiniDevExt->pResetWorkItem, 0);
...
NTSTATUS HumQueueResetWorkItem(IN PDEVICE_OBJECT pDeviceObject, IN PIRP pIrp)
{
    ...
    if ((int)HumIncrementPendingRequestCount(pMiniDevExt) < 0)//NT_SUCCESS(HumIncrementPendingRequestCount(pMiniDevExt)) == FALSE
        return STATUS_SUCCESS;
