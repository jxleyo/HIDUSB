...
HidFx2EvtUsbInterruptPipeReadComplete(WDFUSBPIPE Pipe, WDFMEMORY Buffer, size_t NumBytesTransferred, WDFCONTEXT  Context)
{
    ...
    if (NumBytesTransferred == 0) {//测试结果恒定为5,数据为鼠标集合，或者为10触摸板集合//sizeof(PTP_REPORT)
...
HidFx2EvtInternalDeviceControl(IN WDFQUEUE Queue,IN WDFREQUEST Request,IN size_t OutputBufferLength,IN size_t InputBufferLength,IN ULONG IoControlCode)
{
    ...
    case IOCTL_HID_GET_FEATURE:
        RegDebug(L"IOCTL_HID_GET_FEATURE ok", NULL, runtimes_ioctl_count);
...
HidFx2GetReportDescriptor(IN WDFDEVICE Device,IN WDFREQUEST Request)
{
    ...
    status = WdfMemoryCopyFromBuffer(memory, 0, (PVOID)&ParallelMode_PtpReportDescriptor, bytesToCopy);//ParallelMode_PtpReportDescriptor//MouseReportDescriptor
    if (!NT_SUCCESS(status)) {
...
VOID
DispatchReadRequest(IN PDEVICE_EXTENSION devContext)
{
    ...
    // Set result
    WdfRequestSetInformation(ptpRequest, sizeof(struct mouse_report_t));//sizeof(struct mouse_report_t)//sizeof(PTP_REPORT)
